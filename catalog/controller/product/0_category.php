...
class ControllerProductCategory extends Controller {
	public function index() {
        ...
        // Custom modification: update product prices in the category listing if price is zero
        if (isset($data['products']) && is_array($data['products'])) {
            foreach ($data['products'] as &$product) {
				// Custom modification: handle product price logic similar to product.php
				if (isset($product['price_no_format']) && is_numeric($product['price_no_format'])) {
					$price = (float)$product['price_no_format'];
					$rounded_price = round($price / 100) * 100;
					$product['price_no_format'] = $rounded_price;
					$product['price'] = $this->currency->format($rounded_price, $this->session->data['currency']);
					error_log('Rounded Product Price in Category: ' . $rounded_price);
				} else if (isset($product['price_no_format']) && (float)$product['price_no_format'] == 0) {
					$product['price'] = 'уточнюйте';
					$product['price_no_format'] = 'уточнюйте';
				}
				// Debug: Log the original product price
				if (isset($product['price_no_format'])) {
					error_log('Original Product Price: ' . $product['price_no_format']);
					$product['debug_info'] = 'Original Price: ' . $product['price_no_format'];
				}
                if (isset($product['price_no_format']) && (float)$product['price_no_format'] == 0) {
                    $product['price'] = 'уточнюйте';
                    $product['price_no_format'] = 'уточнюйте';
                }
            }
        ...
        // Ensure modified product data is not overwritten by any subsequent calls
    ...
    // Custom modification: Final check to set price to 'уточнюйте' before rendering
    if (isset($data['products']) && is_array($data['products'])) {
        foreach ($data['products'] as &$product) {
				// Custom modification: handle product price logic similar to product.php
				if (isset($product['price_no_format']) && is_numeric($product['price_no_format'])) {
